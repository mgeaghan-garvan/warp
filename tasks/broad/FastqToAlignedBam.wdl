...
workflow FastqToAlignedBam {
  ...
  String bwa_commandline = "bwa mem -K 100000000 -p -v 3 -t 16 -Y $bash_ref_fasta -R '@RG\\tID:~{sample_and_fastqs.RGID}\\tSM:~{sample_and_fastqs.sample_name}\\tPL:~{sample_and_fastqs.RGPL}\\tPU:~{sample_and_fastqs.RGPU}\\tLB:~{sample_and_fastqs.RGLB}\\tCN:~{sample_and_fastqs.RGCN}'"

  Int compression_level = 2

  # Map reads to reference
  if (use_bwa_mem) {
    call Alignment.FastqAndBwaMemAndMba as FastqAndBwaMemAndMba {
      input:
        input_R1 = sample_and_fastqs.input_R1,
        input_R2 = sample_and_fastqs.input_R2,
  ...
  if (!use_bwa_mem) {
    call DragmapAlignment.FastqAndDragmap as FastqAndDragmap {
      input:
        input_R1 = sample_and_fastqs.input_R1,
        input_R2 = sample_and_fastqs.input_R2,
        RGID = sample_and_fastqs.RGID,
...
# File output_aligned_bam = select_first([SamToFastqAndBwaMemAndMba.output_bam, SamToFastqAndDragmapAndMba.output_bam, SplitRG.aligned_bam])
  File output_aligned_bam = select_first([FastqAndBwaMemAndMba.output_bam, FastqAndDragmap.output_bam])
...
#      output_bam_prefix = sample_and_fastqs.base_file_name + ".readgroup",
#      preemptible_tries = papi_settings.preemptible_tries
#  }
